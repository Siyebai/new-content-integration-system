...
if (require.main === module) {
  const s = new Metacognition_engine(20350, 68, 'metacognition_engine');
  s.start();
  require('http').createServer((req, res) => {
    res.writeHead(200); res.end(JSON.stringify(s.getStatus()));
  }).listen(parseInt(process.env.PORT) || 20350);
}
module.exports = Metacognition_engine;
