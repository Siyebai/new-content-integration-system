...
if (require.main === module) {
  const s = new Communication_theory(20334, 64, 'communication_theory');
  s.start();
  require('http').createServer((req, res) => {
    res.writeHead(200); res.end(JSON.stringify(s.getStatus()));
  }).listen(parseInt(process.env.PORT) || 20334);
}
module.exports = Communication_theory;
