...
if (require.main === module) {
  const s = new Distributed_decision(20348, 67, 'distributed_decision');
  s.start();
  require('http').createServer((req, res) => {
    res.writeHead(200); res.end(JSON.stringify(s.getStatus()));
  }).listen(parseInt(process.env.PORT) || 20348);
}
module.exports = Distributed_decision;
