...
if (require.main === module) {
  const s = new Wormhole_calculator(20319, 61, 'wormhole_calculator');
  s.start();
  require('http').createServer((req, res) => {
    res.writeHead(200); res.end(JSON.stringify(s.getStatus()));
  }).listen(parseInt(process.env.PORT) || 20319);
}
module.exports = Wormhole_calculator;
