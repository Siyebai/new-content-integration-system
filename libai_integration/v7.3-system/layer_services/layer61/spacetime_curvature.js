...
if (require.main === module) {
  const s = new Spacetime_curvature(20315, 61, 'spacetime_curvature');
  s.start();
  require('http').createServer((req, res) => {
    res.writeHead(200); res.end(JSON.stringify(s.getStatus()));
  }).listen(parseInt(process.env.PORT) || 20315);
}
module.exports = Spacetime_curvature;
