...
if (require.main === module) {
  const s = new Omniscience_simulator(20360, 70, 'omniscience_simulator');
  s.start();
  require('http').createServer((req, res) => {
    res.writeHead(200); res.end(JSON.stringify(s.getStatus()));
  }).listen(parseInt(process.env.PORT) || 20360);
}
module.exports = Omniscience_simulator;
